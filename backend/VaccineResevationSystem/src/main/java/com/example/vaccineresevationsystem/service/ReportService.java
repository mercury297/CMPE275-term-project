...
//            System.out.println("start  time after 12"+getDateAfter12Months(startDate));
        ...
        for (Appointment appointment: appointments) {
            if (dateFormat.parse(appointment.getAppointmentDate()).compareTo(newStartDate) >= 0) {
                if (dateFormat.parse(appointment.getAppointmentDate()).compareTo(newEndDate) <= 0) {
                    totalAppointments++;
        ...
        HashMap<String,String> patientReport = new HashMap<>();
        patientReport.put("noShow",String.valueOf(noShowCount));
        patientReport.put("totalAppointments", String.valueOf(totalAppointments));
        float noShowRate = (float)noShowCount/totalAppointments *100;
        patientReport.put("noShowRate", String.valueOf(noShowRate));
    ...
    public ResponseEntity<?> getClinicReport(String startDate,String endDate, String currentTime, String clinicId) throws ParseException {
        ...
        clinincNoShowRate.put("totalAppointments", String.valueOf(totalAppointments));
        float noShowRate = (float) noShowCount/totalAppointments*100;
